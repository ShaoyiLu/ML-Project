...
BEST_PATH = os.path.join(SAVE_DIR, "merged_image_gate_best.pth")
LAST_PATH = os.path.join(SAVE_DIR, "merged_image_gate_last.pth")
...
def train_merged(
    ...
    global_criterion = nn.CrossEntropyLoss(label_smoothing=0.04)
    domain_criterion = nn.CrossEntropyLoss(label_smoothing=0.03)
    slot_criterion = nn.CrossEntropyLoss(label_smoothing=0.03)
    direct_criterion = nn.CrossEntropyLoss(label_smoothing=0.04)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for x, y, slot, domain in train_loader:
            ...
            loss = (
                1.00 * global_criterion(global_logits, y)
                + 0.90 * domain_criterion(domain_logits, domain)
                + 0.35 * slot_criterion(slot_logits, slot)
                + 0.35 * direct_criterion(direct_logits, y)
            )
        ...
        if score > best_acc + 0.03:
            ...
            print(f"Saved new best merged model: {best_acc:.2f}%")
        else:
            bad_epochs += 1
        ...
        if bad_epochs >= patience:
            print("Early stopping: merged accuracy stopped improving.")
            break

    model.load_state_dict(best_state)
    print(f"\nBest merged acc = {best_acc:.2f}% at epoch {best_epoch}")
...
def main():
    ...
    print(f"\nFinal Merged Test Accuracy: {final_acc:.2f}%")
    print(f"Final Merged Test-TTA Accuracy: {final_tta_acc:.2f}%")
    print(f"Final Merged TestDomain Accuracy: {final_domain:.2f}%")
    print(f"Final Merged TestTTA-Domain Accuracy: {final_tta_domain:.2f}%")
    print(f"Final Merged TestSlot Accuracy: {final_slot:.2f}%")
    print(f"Final Merged TestTTA-Slot Accuracy: {final_tta_slot:.2f}%")
    ...
    torch.save(
        {
            ...
            "note": "Image-aware gate fusion. No source-domain input is used during inference.",
        },
        BEST_PATH,
    )

    print(f"\nSaved merged model to {BEST_PATH}")
